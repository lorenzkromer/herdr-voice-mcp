...
export function createMcpServer(ctx: ToolContext): McpServer {
  ...
  server.registerTool(
    "spawn",
    {
      ...
      description:
        "Starts a new coding agent (e.g. claude or codex) in an allowed project: by default a new tab in the project's open workspace (or a new workspace if none is open), working directory = project root; " +
        "with placement=workspace in its own new workspace, with placement=worktree in its own new workspace on its own Git branch, grouped under the project in Herdr. " +
      ...
      inputSchema: {
        ...
        placement: z
          .enum(["tab", "workspace", "worktree"])
          .optional()
          .describe(
            "Where the agent starts. tab (default): a new tab in the project's open workspace. " +
              "workspace: its own new workspace, same project directory. " +
              "worktree: its own new workspace with its own Git branch and checkout, grouped under the project's main workspace in Herdr (like feature workspaces).",
          ),
        label: z.string().optional().describe("Tab or workspace label in Herdr (default: the agent name). A new workspace never gets a label that sounds like an existing one."),
        branch: z.string().optional().describe("placement=worktree only: branch name for the new checkout (default: derived from the label)."),
  ...
  async function doSpawn(args: { project: string; kind?: string; name?: string; prompt?: string; label?: string; placement?: "tab" | "workspace" | "worktree"; branch?: string; request_id?: string }): Promise<CallToolResult> {
    ...
    const placement = args.placement ?? "tab";
    const primary = findProjectWorkspace(workspaces, existing, p.key, p.project, root);
    // A new workspace must not look like an existing one ("Herdr Voice MCP" vs "herdr-voice-mcp"),
    // or workspace-label targets become ambiguous. Generated labels get a suffix; explicit ones are refused.
    const labelTaken = (l: string) => workspaces.some((w) => normalizeQuery(w.label) === normalizeQuery(l));
    let label = args.label ?? name;
    if (placement !== "tab" && labelTaken(label)) {
      if (args.label) return text(`A workspace named like "${args.label}" already exists. Choose another label.`, undefined, true);
      let n = 2;
      while (labelTaken(`${name}-${n}`)) n++;
      label = `${name}-${n}`;
    }
    let branch: string | null = null;
    if (placement === "worktree") {
      branch = args.branch ?? slugify(label);
      if (!BRANCH_RE.test(branch) || branch.includes("..")) return text(`Invalid branch name "${branch}". Use letters, digits, ".", "_", "-" and "/".`, undefined, true);
    }
    ...
    let wsLabel: string;
    let dir = root;
    let created: { kind: "tab" | "workspace" | "worktree"; id: string; path?: string; branch?: string };
    try {
      if (placement === "worktree") {
        // Herdr groups the worktree workspace under the project's primary workspace.
        const opts = primary?.worktree && !primary.worktree.is_linked_worktree ? { workspace_id: primary.workspace_id } : { cwd: root };
        const w = await client.worktreeCreate({ ...opts, branch: branch!, label }, Math.max(5_000, deadline - Date.now() - SPAWN_MIN_HANDOVER_MS - 3_000));
        pane_id = w.root_pane.pane_id;
        wsLabel = w.workspace.label;
        dir = w.worktree.path;
        where = `new worktree workspace "${wsLabel}" (branch ${w.worktree.branch}) under ${primary ? `"${primary.label}"` : p.project.name}`;
        created = { kind: "worktree", id: w.workspace.workspace_id, path: w.worktree.path, branch: w.worktree.branch };
      } else if (placement === "tab" && primary) {
        ...
        // placement "workspace", or "tab" while no workspace of the project is open.
        const wsName = placement === "workspace" ? label : path.basename(root);
        const w = await client.workspaceCreate(wsName, root);
        pane_id = w.root_pane.pane_id;
        wsLabel = w.workspace.label;
